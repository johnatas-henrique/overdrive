...
5. Update Fuel and Tire runtime state from `TickStartSnapshot` and the corresponding `ResolvedCarInput[carId]`, as two ordered sub-steps: **5a — FuelSystem.Tick()** consumes throttle from Step 2 and applies refuel rate when `PitServiceCommand[carId].active`; **5b — TireSystem.Tick()** consumes resolved steering plus speed/surface contact for wear and resets wear after 2s of service when `PitServiceCommand[carId].active`. Per-tick consumption/wear applies its per-second formula multiplied by `FIXED_DT`; Countdown and Qualifying explicitly skip both.
...
9b. **PitStopSystem.Tick()** reads `CarState[carId].PitPhase` and the Track pit geometry from `TickStartSnapshot`, advances the pit-service timer, and writes `PitServiceCommand[carId]` for FuelSystem/TireSystem to consume at Step 5a/5b of the next tick. It never mutates Fuel or Tire state directly.
...
14. Simulation resolves `ResolvedCarInput[carId]` for the next tick from the cached `AIInput` prepared by Step 13. The player's input is processed at Step 2 of each tick from that tick's captured raw sample (ADR-0005 Single Frame-Level Capture) — Step 14 never re-carries a raw sample; the next tick's ResolvedCarInput combines the player's fresh Step 2 processing with the AI's cached decision.
...
|----------|--------|---------------|-----------|
| `TickStartSnapshot` | Prior `simulationStepCount`, `activeRaceStepCount`, `sim_time`, `SimulationState`, `RaceMode`, `countdownRemainingTicks`; ascending-`carId` prior `CarState[]`, Fuel state[], Tire state[], `PitServiceCommand[]`, RSM position/lap/rival-gap state[]; cached `AIInput[]` | Step 1; values except cached AIInput copy from the prior PublishedSimulationSnapshot; cached AIInput comes from Step 13 of the prior tick | Input, Fuel, Tire, Vehicle Physics, Pit Stop |
| `PublishedSimulationSnapshot` | Current `simulationStepCount`, `activeRaceStepCount`, `sim_time`, `SimulationState`, `RaceMode`, active immutable `DifficultyProfile` ID, `countdownRemainingTicks`; ascending-`carId` current `CarState[]`, Fuel state[], Tire state[], Pit state[], `PitServiceCommand[]`, RSM position/lap/rival-gap state[]; when Paused: Simulation-owned `resumeState`; `PerformanceReduced { severity, observedFps }` when active; when Finished or Results after a normal/qualifying completion: RSM-owned copies of `resultKind`, `resolutionComplete`, `playerFinishTime`, `terminalPresentationRequest`, and `ResolvedFinishOrder`; when Results follows a forfeit: `resultClassification = Forfeit`, `forfeitLapCount`, and `raceTimeAtForfeit` | Step 12 for active ticks; lifecycle transition for Idle, Loading, Paused, Finished, or Results | AI on active ticks, HUD, Camera, RSM next tick, UI Presentation, Pit Stop next tick |
...
| From | To | Trigger |
|------|----|---------|
| `Idle` | `Loading` | Qualifying Results sends `StartRaceRequested`; RSM returns `TransitionRequest(Loading, RaceStartRequested, gridAssignment)` and Simulation sends the corresponding ContentLoadRequest |
...
| `Finished` | `Results` | Resolution is complete and player dismisses terminal presentation; `resultKind` selects Race Results or Qualifying Results |
| `Results` | `Loading` | Race Results starts another race, or Qualifying Results sends `StartRaceRequested`; RSM returns a Loading TransitionRequest and Simulation sends the matching ContentLoadRequest (Content Pipeline calls this `Race Reconfigure`) |
...
- **Loading → Racing (Qualifying):** Simulation initializes `accumulator = 0`, the same clocks and local state, then Race Session Manager supplies `RaceMode.Qualifying`; the qualifying car spawns at the pit box with `GameplayQualifying` and drives an out-lap through pit lane (no fuel/wear, timer off) before the flying lap. No Countdown state, grid lock, lights sequence, or pre-GO tick occurs for Qualifying.
...
| **Pit Stop** | Bidirectional | `CarState[].PitPhase` in; `PitServiceCommand[]` out | Step 9b of every active tick | Simulation supplies the prior tick's PitServiceCommand in TickStartSnapshot; PitStopSystem reads CarState PitPhase, advances the service timer, and writes a fresh PitServiceCommand for the next tick. Pit entry remains physical; no input action initiates service. |
...
| Pit Stop | Bidirectional | Hard | Simulation → Pit Stop: `CarState[].PitPhase` and Track pit geometry via `TickStartSnapshot` at Step 9b. Pit Stop → Simulation: `PitServiceCommand[]` written at Step 9b and carried to the next tick's snapshot for FuelSystem/TireSystem at Steps 5a/5b. Pit Stop never mutates Fuel or Tire state directly. |
