...
The audio system mixes 4 independent layers. Each layer has its own volume control routed through the Settings mixer groups. Layers stack additively.

| # | Layer | Content | Update Rate | Mixer Group |
|---|-------|---------|-------------|-------------|
| 1 | **Engine** | Procedural engine oscillator (RPM-based pitch) | Per physics tick (60 Hz) | SFX |
...
**Output Range:** 60 Hz (idle, empty tank) to 16800 Hz (redline, full tank). Clamped to audible range 40–20000 Hz.
...
Tire squeal is a continuous sound triggered by grip loss. Volume and frequency scale with wear. Pitch stays constant.

| Parameter | Formula | Description |
|-----------|---------|-------------|
| **Trigger** | `squeal_active = (grip_loss > 0.15) AND (speed > 30 km/h)` | Active when lateral slide exceeds threshold |
| **Volume** | `squeal_vol = grip_loss × (wear_percent / 100) × sfx_volume` | Louder with more wear |
| **Pitch** | `squeal_pitch = 1200 Hz` | Constant — not affected by wear |
| **Frequency** | `squeal_freq = 0.1 + 0.9 × (wear_percent / 100)` | More frequent with wear |
...
**Output Range:**
- 0% wear: squeal at 1200 Hz, low volume, only in hard corners
...
| **Countdown** | Race start 3-2-1 | Countdown beeps | Fixed volume 0.9 |
...
Low-speed tap: 200–400 Hz (thud). High-speed slam: 1200–1000 Hz (crunch).
...
| **Pit** | Active (pit-lane RPM cap) | Pit SFX only, wind off | Background track (ducked) | Active |
| **Menu** | Off | UI sounds only | Menu music | Off |
| **Countdown** | Idle RPM (800) | Countdown beeps | Background track (low) | Active |
| **Post-Race** | Off | Finish sting | Results music | Crowd fade-in |
...
- Tire squeal frequency increases (more sliding)

### States and Transitions

| From | To | Trigger | Audio Change |
|------|----|---------|--------------|
| Menu | Countdown | Race start initiated | Menu music fades, engine idle starts |
| Countdown | Race | "GO" signal | Race start sting, full engine, all SFX active |
| Race | Pit | Car enters pit lane | Pit entry sting, engine RPM capped, wind off, pit SFX |
| Pit | Race | Pit stop complete | Engine revs, tire chirp, wind returns |
| Race | Post-Race | Cross finish line | Finish sting, engine fades over 3s, results music |
| Race | Qualifying | Session change | Stings disabled, background track continues |
| Qualifying | Race | Session change | Stings enabled |
| Post-Race | Menu | Player exits results | Results music fades, menu music starts |
...
| **Settings** | Inbound | Master, Music, SFX, UI volumes, Mute state | AudioSettings | On apply (immediate) |
...
**Output Range:** 40 Hz to 20000 Hz (clamped). Typical: 200 Hz (idle) to 8000 Hz (redline).

### Tire Squeal Volume

`squeal_vol = grip_loss × (wear_percent / 100) × sfx_volume`

**Output Range:** 0.0 (no slide, new tires) to 1.0 (full slide, bald tires).
...
- **GIVEN** a car at 8000 RPM in 4th gear, **WHEN** engine sound is played, **THEN** pitch is approximately 5600 Hz.
...
- **GIVEN** tire at 75% wear and grip_loss > 0.15, **WHEN** squeal is active, **THEN** volume is at maximum and squeal is constant in corners.
- **GIVEN** tire at 0% wear and grip_loss > 0.15, **WHEN** squeal is active, **THEN** volume is low and squeal only in hard corners.
...
- **Engine sound per car:** Should each team have a distinct engine character (different cylinder count, different timbre)? Or is RPM-based pitch sufficient for differentiation?
...
- **Pit stop audio:** Should the pit stop have a unique music sting, or is the existing pit entry sting sufficient?
