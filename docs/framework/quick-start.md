...
4b. **Enhance with unity-mcp (Unity only)** — Install the CoplayDev unity-mcp
    ...
    ```
Then add the `mcp.unity` block to `opencode.json` and set
...
### 6. Use the OpenCode Runtime

The OCGS framework is already installed directly under `.opencode/`.
Run `opencode` from the project root, then type `/` to browse all skills and
commands or `/start` for onboarding.
