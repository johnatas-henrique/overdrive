...
|-----------|----------|---------|
| **Agents** | `.opencode/agents/` | Agent definitions (51 files) |
| **Skills** | `.opencode/skills/` | Skill workflows (77 directories) |
| **Commands** | `.opencode/commands/` | Slash commands (54 files) |
...
For framework documents, verify frontmatter, required sections, and
cross-references manually. Automated validation is reserved for active plugins.
...
Review frontmatter, required sections, and agent cross-references manually.

### For Skill Changes

Verify the skill parses correctly, phases are executable, and cross-references
are valid by reading the affected skill, command, and referenced agent files.
...
If you add gameplay code, write Unity Test Framework tests under `tests/` and
run the relevant Unity test suite.
