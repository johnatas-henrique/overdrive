...
- [ ] **AC-4.8:** Given the game is in Racing state and the player crosses the finish line on the final lap, When the finish condition is detected, Then the state transitions to Finished, no further gameplay simulation tick executes (race controls are no longer routed to simulation — the literal input context switch is Input/UI epic scope, the Kernel verifies the state transition + snapshot carrying `terminalPresentationRequest` per AC-4.8d), and FinishOrderResolver runs once from the locked snapshot.
- [ ] **AC-4.8dnf:** Given Racing detects fuel empty and the player car stopped on track, When RSM returns FinishDetected with player DNF, Then Simulation enters Finished and passes the locked PostFinishSnapshot (player classification DNF) to FinishOrderResolver once without waiting for live simulation. *(The resolver's projection of every non-DNF unfinished AI is RSM-owned — verified in the RSM epic's contract test; the Kernel verifies the resolver is invoked once with the DNF-classified snapshot and the result is consumed without further ticks.)*
- [ ] **AC-4.8a:** Given `resultKind = Race` or `Qualifying`, When resolution is complete and terminal presentation is dismissed, Then Simulation transitions from Finished to Results without a physics tick. *(`resultKind` is published in the snapshot per AC-4.8d so the result-specific UI can select — the literal UI opening is UI Menu epic scope, DEFERRED to the UI gate.)*
- [ ] **AC-4.8b:** Given `resultKind = Qualifying`, When the flying lap completes or fails, Then `resolutionComplete` is immediately true and the terminal presentation request is published. *(Confirm or its 5-second timeout transitions Simulation to Results — the Kernel consumes the dismissal; "opens Qualifying Results" is UI Menu epic scope, DEFERRED to the UI gate.)*
...
- [ ] **AC-4.8h:** Given Simulation is in Results with `resultKind = Qualifying`, When the Kernel consumes RSM's `TransitionRequest(Loading, QualifyingComplete, gridAssignment)` (emitted after Qualifying Results confirms Start Race — the confirmation itself is UI → RSM), Then Simulation enters Loading, sends `ContentLoadRequest(Race, gridAssignment)`, and transitions to Countdown only after `RaceLoadReady(RaceMode.Race, gridAssignment)`.
...
- [ ] **AC-4.10:** Given the game is in Results state, When the player selects "Next Race", Then the Kernel consumes RSM's `TransitionRequest(Loading, gridAssignment)` and the state transitions to Loading (the request issued once; no simulation tick before a valid `RaceLoadReady`).
...
- [ ] **AC-4.10b:** Given Simulation is waiting in Results after `ContentUnloadRequest`, When Content emits `ContentUnloadComplete`, Then Simulation publishes Idle and retains no Kernel-owned session references (race mode, grid, resolved order). *(Addressables asset release is Content-owned — verified in the Content epic; the Kernel verifies its own references are cleared.)*
- [ ] **AC-4.12:** Given a Paused state whose `resumeState` is Countdown or Racing, When the player selects Return to Menu, Then Simulation transitions directly to Results without a resumed physics tick, and the published Results snapshot carries RSM-originated forfeit metadata (`resultClassification = Forfeit`, `forfeitLapCount`, `raceTimeAtForfeit`) with no final position. *(RSM publishes the metadata and emits `RaceAborted(Forfeit)`; Ghost Recording's buffer discard is Story 008 consuming that event — both external, verified in their own epics.)*
...
- AI skip observable seam (AC-4.8j): the Kernel-owned `ResolvedCarInput.AI` (SimulationContracts) is the cache seam — the pipeline produces it during Racing ticks; on the finishing tick that publishes Finished, Step 13 skips AI evaluation and the published snapshot carries no cached AIInput; later Finished/Results updates produce none. Tests assert the snapshot carries empty AI and no new AI is produced after the result resolves.
...
- **AC-4.8**: finish detected → Finished, no further tick, resolver once
  ...
  - Then: state becomes Finished; no further gameplay simulation tick executes; FinishOrderResolver invoked exactly once with locked post-finish snapshot
  - Edge cases: duplicate FinishDetected does not re-resolve; finish on non-final lap does not transition; snapshot carries terminalPresentationRequest (AC-4.8d)
...
- **AC-4.8dnf**: fuel empty DNF resolution
  ...
  - Then: enters Finished; FinishOrderResolver invoked once with the DNF-classified locked snapshot; result consumed without waiting for another tick
  - Edge cases: existing AI DNFs remain DNF (RSM contract); fuel exactly zero while moving does not trigger retirement (RSM/vehicle-detection rule — verified in the RSM epic's contract test, NOT here)

- **AC-4.8a**: Finished→Results without physics tick, resultKind published — gated on dismissal
  - Given: resultKind Race or Qualifying; resolution complete
  - When: terminal presentation dismissed
  - Then: transitions Finished→Results without a physics tick; snapshot carries resultKind (AC-4.8d) for UI selection
...
- **AC-4.8b**: Qualifying flying lap → immediate resolution, Confirm/timeout → Results (not unconditional)
  ...
  - Then: resolutionComplete true immediately; terminal presentation request published; Confirm or 5s timeout transitions to Results
  - Edge cases: Confirm before lap resolution does not transition; no unconditional immediate Finished→Results; failed lap still produces resolved qualifying result (RSM qualifying-resolution evidence — the Kernel verifies only that it consumes the returned result and snapshot)
...
- **AC-4.8e**: Kernel resolver-interface constraint
  ...
  - Edge cases: RSM tie-break ordering and DNF projection are RSM epic internals — verified in the RSM epic's contract test, NOT here; zero remaining distance does not change classification (resolver/RSM behaviour — verified in the RSM epic's contract test, NOT here)
...
- **AC-4.8h**: Qualifying Results Start Race → Loading → Countdown after RaceLoadReady
  - Given: Results with resultKind Qualifying; RSM mock returns TransitionRequest(Loading, QualifyingComplete, gridAssignment)
  - When: Simulation consumes the TransitionRequest
...
- **AC-4.8i**: Finished timer — Kernel state invariant
  ...
  - Edge cases: timer countdown/expiry behavior is UI Menu epic scope (DEFERRED); exactly zero remaining time dismisses once (expiry semantics UI-owned — the Kernel verifies only dismissal-event idempotency: a single emitted dismissal causes exactly one transition); stale/duplicate dismissal causes no additional transition
...
- **AC-4.8j**: Step 13 skipped in Finished, stale AI cache cleared
  ...
  - Observable seam: the Kernel-owned `ResolvedCarInput.AI` (SimulationContracts) produced by the pipeline — in Finished/Results the published snapshot carries no cached AIInput (stale cache from a prior Racing tick cleared); the pipeline never produces new AI after the result resolves
...
- **AC-4.10**: Results Next Race → Loading
  - Given: Results
  - When: RSM mock returns TransitionRequest(Loading, gridAssignment)
  - Then: transitions to Loading; issues ContentLoadRequest once
...
- **AC-4.10b**: ContentUnloadComplete → Idle, no Kernel references retained
  ...
  - Then: publishes Idle; Kernel-owned session references (race mode, grid, resolved order) cleared
  - Edge cases: incomplete unload leaves Results; duplicate completion harmless; Addressables release is Content-owned
...
- **AC-4.12**: Paused (resumeState Countdown/Racing) + Return to Menu → Results forfeit
  ...
  - Then: transitions directly to Results without resumed physics tick; published Results snapshot carries RSM-originated resultClassification=Forfeit, forfeitLapCount, raceTimeAtForfeit with no final position
  - Edge cases: Countdown → lap count 0, race time 0.0; Racing → paused lap count and activeRaceStepCount × FIXED_DT (exact lap/time values are RSM-owned metadata generation — the Kernel verifies transport of the received metadata and that no resolver is invoked); no finish order created; RSM metadata publication + RaceAborted(Forfeit) emission are RSM-owned (Story 008 consumes the discard)
...
`SessionEndTests.cs` (55 tests, 305/305 PlayMode green). Verifies: finish detection/resolution (4.8, 4.8dnf, 4.8e, 4.8f), terminal presentation + dismissal (4.8a, 4.8b, 4.8c, 4.8i, 4.8g), snapshot immutability (4.8d), AI skip (4.8j), results/forfeit/unload (4.8h, 4.10, 4.10a, 4.10b, 4.12), finished focus retention (7.1c).
...
**Criteria**: 17/17 passing (0 deferred)
**Deviations**:
- ADVISORY (AC-4.8e): Fuel/Tire/Pit/collision/tactical-AI step non-execution is proven at the Kernel boundary (LoggingStep spies + `SimulationStepCount` unchanged on Finished frames); the per-step end-to-end assertion is deferred to those epics' own step tests (TD-018).
- ADVISORY (Schema): `PostFinishSnapshot.ResultClassification` is always empty — no consumer reads it; either populate from `ResolvedFinishOrder` or remove (TD-019).
**Test Evidence**: `Assets/tests/integration/simulation/SessionEndTests.cs` — 55 tests, 305/305 PlayMode green (262 prior + 55 new, zero regressions).
**Code Review**: Complete (unity-specialist + qa-tester, 4 rounds: R1 1 BLOCKING + 10 GAPS, R2 APPROVED + 5 GAPS, R3 2 BLOCKING + 3 GAPS, R4 TESTABLE 0 BLOCKING; QL-TEST-COVERAGE ADEQUATE R3; LP-CODE-REVIEW APPROVED).
