...
- [ ] AC-6a: When `LoadAssetContainerAsync` rejects (e.g., 404 on GLB), `'asset.error'` is emitted on Event Bus with `{ assetId: string, error: Error }`.
...
- [ ] AC-10b: `'asset.load.progress'` is emitted during `load()` with `{ id: string, loaded: number, total: number }` — fires at least once.
- [ ] AC-10c: `'asset.load.complete'` is emitted on successful load with `{ id: string }`.
- [ ] AC-10d: When loading from cache (second `load('spa')`), all three events (`start`, `progress`, `complete`) fire identically to a first load.
...
1. **Event emission order** — For a single asset load: `'asset.load.start'` → `'asset.load.progress'` → `'asset.load.complete'`
2. **Progress values** — For a single-GLB load, `loaded: 1, total: 1`. For batch (Story 005a), cumulative across all assets.
3. **Cache-hit events** — Even when returning from cache, emit all three events so the loading screen has a consistent event model. Progress fires synchronously.
4. **Error handling** — Wrap the `LoadAssetContainerAsync` call in try/catch. On error, emit `'asset.error'` and re-throw so the caller knows the load failed.
5. **Texture missing** — Babylon.js logs a warning and shows a checkerboard pattern. Detect via `console.warn` interception or `scene.onTextureLoadingErrorObservable` if available. Emit `'asset.error'` with the texture ID.
6. **Required imports**: `import "@babylonjs/loaders/glTF/2.0/glTFLoader"` (side-effect import — without it, GLB loading silently fails).
...
- Given: Initialized AssetManager, mocked `LoadAssetContainerAsync` that rejects with `Error('404 Not Found')`, spy subscriber on `'asset.error'`
...
**Required evidence**: `tests/integration/asset-manager/story-003-loading-events_test.ts` OR playtest doc
