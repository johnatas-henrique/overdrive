...
- Contains: Unity C# scripts, scenes, materials, sprites, input actions, render pipeline assets
  - `Overdrive.Input` (`Assets/source/Overdrive.Input.asmdef`): Unity-side input processing — `InputContextController` (partial class split across `InputContextController.cs`, `.Contexts.cs`, `.Latches.cs`, `.Scheme.cs`), `DeadZoneNormalizer`, `EmaBrakePriority`, `ControlProfile`, `TickProcessor`, `InputFrameCapture` (production capture adapter), `InputBindingCatalog` (binding slot indexing with override-cap accounting, stale-index repair, `GetActionId`, `HasBinding`, `RefreshSlotPath`), `EmaReinitializerBase`, `ContextResumeEmaReinitializer`, `SchemeChangeEmaReinitializer`, `SettingsInputPreviewEvaluator`
  ...
  - `Overdrive.Simulation` (`Assets/source/Simulation/Overdrive.Simulation.asmdef`): Engine-free simulation kernel — `SimulationKernel` (14-step invocation spine), `SimulationStateMachine` (authoritative session lifecycle), `SimulationDriver` (manual fixed-step accumulator), `GhostRecorderLifecycle` (ghost-recorder and GO-boundary replay-capture lifecycle), `RenderInterpolator` (render interpolation math), `PerformanceMonitor` (FPS protection), `Pcg32` (deterministic PRNG), `GhostBuffer` (ghost recording), `DeterminismHarness` (determinism replay), `ICanonicalSpineStep` (canonical slot identity for pipeline steps — kernel enforces correct slot at construction, fails fast on mismatch), `IPhysicsSimulator` (engine boundary seam for physics stepping), contract spine types (`CarState` with 3D pose, `DifficultyProfile` with 6 immutable fields, `FuelState`, `TireState`, `GridAssignment`, `SimulationTickContext`, `PublishedSimulationSnapshot`, `PostFinishSnapshot`, pipeline steps). Depends on `Unity.Mathematics`; has `noEngineReferences: true`
  - `Overdrive.Simulation.Unity` (`Assets/source/Simulation.Unity/Overdrive.Simulation.Unity.asmdef`): Unity lifecycle adapters for the simulation driver — `UnityFrameDeltaSource` (unscaled render delta), `UnityPhysicsSimulator` (freezes automatic physics, performs explicit `Physics.Simulate` step), `SimulationDriverBehaviour` (MonoBehaviour Update entry point for the engine-free `SimulationDriver`). Namespace `Overdrive.Simulation.UnityAdapters`; references only `Overdrive.Simulation`; `noEngineReferences: false`. One-way dependency: Simulation.Unity → Simulation.
...
- Contains: EditMode and PlayMode test directories, unit tests (`Assets/tests/unit/input/`, `Assets/tests/unit/simulation/`, `Assets/tests/unit/multiplayer/`, `Assets/tests/unit/settings/`, `Assets/tests/unit/content/`), integration tests (`Assets/tests/integration/input/`, `Assets/tests/integration/simulation/`, `Assets/tests/integration/settings/`, `Assets/tests/integration/content/`), smoke tests (`tests/smoke/`); plugin tests in `.opencode/plugins/tests/`
...
- Location: `Assets/source/Simulation/SimulationKernel.cs`
  - Pattern: Engine-free class in the `Overdrive.Simulation` assembly; captures one raw input sample per render frame and reuses it across ticks; executes steps in array order with early-exit on pause boundary; constructor enforces `ICanonicalSpineStep` slot identity so production steps in wrong positions fail fast
...
- Purpose: Engine-free lifecycle controller for the loading screen — minimum-display timing (0.5s), monotonic progress clamping, first-launch "Preparing..." state, terminal exactly-once semantics, input-blocked report.
- Location: `Assets/source/Content/LoadingScreenController.cs`
- Pattern: Engine-free sealed class in `Overdrive.Content`; drives an `ILoadingScreenPresenter` (fire-and-forget visual seam); multi-cycle re-arm via `BeginPreparing`/`BeginLoading`; elapsed display time starts at first `Begin` call; exceptions from presenter are swallowed by SafePublish pattern

**MipmapLimitResolver:**
- Purpose: Pure preset-to-mipmap-limit table deriving texture resolution policy from quality presets.
- Location: `Assets/source/Content/MipmapLimitResolver.cs`
- Pattern: Static class in `Overdrive.Content`; maps `QualityPresetId` to int limit (Low→2, Medium→1, High/Ultra→0, Custom→Medium-equivalent fallback); consumed by `ITextureMipmapApplier` seam in `Overdrive.Content.Unity`
