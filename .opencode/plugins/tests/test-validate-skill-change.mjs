...
// Matches the active OpenCode skill and command paths
...
// ── S2b: Windows-style .opencode/ paths still match ──
{
  ...
  run("S2b: .opencode/ paths — extract name", () => {
...
// ── S3: Returns null for non-skill paths ──
{
  ...
  const r5 = detectSkillChange("/project/legacy/plugins/some-tool.ts")
...
// ── S9: Skills directory outside .opencode/ — not matched ──
{
  const result = detectSkillChange("/project/src/skills/stuff.txt")
  run("S9: skills/ dir outside .opencode — null", () => {
    assert.equal(result, null)
  })
}

// ── S10: Commands directory outside .opencode/ — not matched ──
{
  const result = detectSkillChange("/project/other/commands/foo.md")
  run("S10: commands/ dir outside .opencode — null", () => {
    assert.equal(result, null)
  })
}
