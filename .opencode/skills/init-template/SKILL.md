...
The OpenCode framework is already installed directly under `.opencode/`.
Do not run a module installer. Use the available agents, skills, commands,
rules, and plugins from that runtime.
...
Before any file write, edit, deletion, Git reset, or directory cleanup, present
the exact paths and operations to the user and ask, "May I write this to
[filepath]?" or, for destructive operations, request explicit confirmation.
Do not execute those operations before approval.
...
- **COMPLETE** — all approved initialization steps finished successfully.
- **CONCERNS** — initialization finished with non-blocking warnings.
- **FAIL** — one or more approved initialization steps failed.
