...
- **security-engineer** _(if game has online features, multiplayer, or player data)_: Conduct pre-release security audit. Review authentication, anti-cheat, data privacy compliance. Sign off on security posture.
- **network-programmer** _(if game has multiplayer)_: Sign off on netcode stability. Verify lag compensation, reconnect handling, and bandwidth usage under load.
