...
description: "Read a story file and implement it. Loads the full context (story, GDD requirement, ADR guidelines, control manifest), implements the code and test directly (primary agent) with engine specialists as consultants, and confirms each acceptance criterion. Variant of /dev-story: the orchestrator implements directly instead of spawning a programmer sub-agent (full MCP tool access + real-time user interaction). Run after /story-readiness, before /code-review and /story-done."
...
all the context implementation needs, implements the code and test directly
(the primary/orchestrating agent — it has access to ALL MCP tools and allows the
user to interact in real time), and uses engine specialists as consultants to
validate approach on HIGH-risk stories. It drives implementation to completion.
...
## Phase 3: Route to the Right Specialist (Consultant)

The implementation is done directly by the orchestrator (primary agent). Engine specialists are **consultants** — spawned to validate approach BEFORE implementation on HIGH-risk stories, not to write code. This is because the primary agent has access to ALL MCP tools (sub-agents have limited tool access) and allows real-time user interaction.

**Config/Data stories — no implementation code needed:**
If the story's Type is `Config/Data`, the implementation is a data file edit — no specialist consultation needed. Jump directly to Phase 4 (Config/Data note).

### Specialist consultation routing table

| Story context | Consultant to validate approach with |
|---|---|
| Foundation layer — HIGH engine risk | engine specialist (e.g. `unity-specialist`) |
| Any layer — Type: UI | `ui-programmer` / UI specialist |
| Any layer — Type: Visual/Feel | gameplay consideration — validate feel approach |
| Core or Feature — AI behaviour, pathfinding | `ai-programmer` |
| Core or Feature — networking, replication | `network-programmer` |
| Config/Data — no code | No consultant needed (see Phase 4 Config note) |

### When to consult the engine specialist

Read the `Engine Specialists` section of `docs/framework/technical-preferences.md`
to get the configured primary specialist. Consult them (spawn via Task) when the
story involves engine-specific APIs, patterns, or the ADR has HIGH engine risk.
...
**When engine risk is HIGH** (from the ADR or VERSION.md): always consult the engine
specialist to validate the implementation approach BEFORE writing code, even for
non-engine-facing stories. High risk means the ADR records assumptions about
post-cutoff engine APIs that need expert verification.

---

## Phase 4: Implement (Directly, by the Orchestrator)

The orchestrator implements the code and test directly — do NOT spawn a programmer
sub-agent to write the code. The orchestrator has all MCP tools and can interact
with the user in real time.

**Before implementing**, run `date` to record the phase start (wallclock tracking
per phase). Record the wallclock at the start of each subsequent phase too.
...
- Create or modify files following the ADR guidelines
...
### GDD/ADR Defect Protocol

If you find a defect in the GDD or ADR during implementation (a contradiction,
a naming inconsistency, a formula that conflicts with another section):
1. **STOP** implementation
2. Bring it to the user with your opinion on what is happening and a proposed resolution
3. Fix the source (GDD/ADR) AND all propagated copies (story, TR registry, review logs)
4. Continue implementation only after the user confirms the resolution

A GDD/ADR defect left unfixed propagates to the code and every referencing location.

### Config/Data stories (no implementation code)

For Type: Config/Data stories, no specialist consultation is required. The implementation
...
For **Visual/Feel** and **UI** stories: no automated test. Note in the
implementation summary what manual evidence will be needed:
...
After implementation completes, collect:
...
- Any questions or blockers surfaced
- Any engine-specific risks the specialist consultant flagged
...
If any consulted specialist (via Task) returns BLOCKED, errors, or cannot complete:

1. **Surface immediately**: Report "[Consultant]: BLOCKED — [reason]" to the user before continuing to dependent phases
2. **Assess dependencies**: Check whether the consultant's output is required by subsequent phases. If yes, do not proceed past that dependency point without user input.
3. **Offer options** via question with choices:
   - Skip this consultant and note the gap in the final report
   - Retry with narrower scope
   - Stop here and resolve the blocker first
4. **Always produce a partial report** — output whatever was completed. Never discard work because one consultant blocked.

If the ORCHESTRATOR itself is blocked (cannot find a defect cause, MCP unavailable), do not loop: after 2 failed attempts with the same fix, stop and re-diagnose with evidence (measure first via MCP/script-execute before theorizing).
...
- **File writes are direct** — the orchestrator (primary agent) writes all source code, test files, and evidence docs itself using its full MCP tool access. Engine specialists are consulted for approach validation, not file writing. Consultants do not write files.
