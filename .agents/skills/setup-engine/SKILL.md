...
**MCP Configuration:**
Add the following to your OpenCode MCP config (`opencode.json`):
```json
{
  "mcpServers": {
    "godot": {
...
> **Note:** The wizard's per-client list (Claude Desktop, Cursor, Claude Code, VS Code, Windsurf, Cline, etc.) does not explicitly mention OpenCode. The wizard MAY auto-configure `opencode.json` (verify by checking the file after); if it does not, use the manual config below.

#### Manual MCP config (reliable fallback)

Add this block to the `mcp` object in `opencode.json`:
